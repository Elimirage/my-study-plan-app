...
def is_fundamental(name: str):
    """
    Определяет фундаментальные дисциплины (обязательная часть).
    """
    name = name.lower()
    ...
    fundamental_keywords = [
        # математика
        "математ", "анализ данных", "математический анализ", "статистик", "вероятност",

        # программирование
        "программирован", "python", "алгоритм", "структуры данных",

        # архитектура и системы
        "архитектур", "операционн", "системн",

        # сети
        "сет", "network",

        # инженерия
        "инженер", "автоматическ управление",

        # базы данных
        "базы данных", "sql", "субд",

        # проектная деятельность (базовая)
        "проектн деятельность", "project management",

        # информационные технологии (базовые)
        "информационные технологии", "it",
    ]

    # НЕ фундаментальные (всегда вариативные)
    variative_keywords = [
        ...
        "автомобиль", "диагностик", "интерфейс", "мультимедиа",
        "визуализация", "графическ", "моделирование", "оптимизация"
    ]

    # Если дисциплина явно прикладная — она вариативная
...
def assign_blocks(disciplines):
    """
    Lite определяет только блок:
    - Блок 1. Обязательная часть
    - Блок 1. Вариативная часть
    (Практика и ГИА добавит Python)
    """
    prompt = f"""
Ты — методист вуза РФ.

Для каждой дисциплины определи блок:
- Если block_hint="обязательная" → "Блок 1. Обязательная часть"
- Если block_hint="вариативная" → "Блок 1. Вариативная часть"
...
"""
    ...
    try:
        ...
        return json.loads(raw[start:end]).get("blocks", [])
    except:
        return []
